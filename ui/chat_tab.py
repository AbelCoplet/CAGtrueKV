...
class ChatTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.model_label.setStyleSheet("color: #333333; font-weight: bold;")
        ...
        temp_label.setStyleSheet("color: #333333;")
        ...
        self.temp_value.setStyleSheet("color: #333333;")
        info_layout.addWidget(self.temp_value)
        
        # Max tokens
        max_tokens_label = QLabel("Max Tokens:")
        max_tokens_label.setStyleSheet("color: #333333;")
        ...
        self.chat_history.setStyleSheet(
            "QTextEdit { background-color: white; border: 1px solid #ddd; color: #333333; }"
        )
        ...
        self.message_input.setStyleSheet("color: #333333;")
        ...
        self.token_count_label.setStyleSheet("color: #333333;")
    ...
    def initialize_state(self):
        ...
        # Set current KV cache path label
        current_kv_cache_path = self.chat_engine.current_kv_cache_path
        if self.chat_engine.use_kv_cache:
            if current_kv_cache_path:
                cache_name = Path(current_kv_cache_path).name
                self.kv_cache_label.setText(f"Current KV Cache: {cache_name}")
            else:
                # Check if master cache is configured
                master_cache_path_str = self.config.get('MASTER_KV_CACHE_PATH')
                if master_cache_path_str and Path(master_cache_path_str).exists():
                     self.kv_cache_label.setText(f"Current KV Cache: Master ({Path(master_cache_path_str).name})")
                else:
        ...
        # Set checkbox state
        self.kv_cache_checkbox.setChecked(self.chat_engine.use_kv_cache)
    ...
    def add_user_message_to_display(self, message: str):
        ...
        cursor.insertHtml(
            ...
            f'<div style="background-color: #E3F2FD; border-radius: 5px; padding: 8px; margin: 5px 0px; color: #333333;">'
            f'{message.replace("\n", "<br>")}'
            f'</div></div>'
        )
    ...
    def add_assistant_message_to_display(self, message: str):
        ...
        cursor.insertHtml(
            ...
            f'<div style="background-color: #F1F8E9; border-radius: 5px; padding: 8px; margin: 5px 0px; color: #333333;">'
            f'{message.replace("\n", "<br>")}'
            f'</div></div>'
        )
    ...
    def toggle_kv_cache(self, enabled: bool):
        ...
        if enabled:
            ...
            if current_kv_cache_path:
                ...
                # Check if master cache is configured when enabling
                master_cache_path_str = self.config.get('MASTER_KV_CACHE_PATH')
                if master_cache_path_str and Path(master_cache_path_str).exists():
                     self.kv_cache_label.setText(f"Current KV Cache: Master ({Path(master_cache_path_str).name})")
                else:
    ...
    def on_cache_selected(self, cache_path: str):
        ...
        # This signal comes from CacheTab's "Use Selected" button
        # We need to tell the ChatEngine to use this path
        success = self.chat_engine.set_kv_cache(cache_path)
        if success:
            cache_name = Path(cache_path).name
            self.kv_cache_label.setText(f"Current KV Cache: {cache_name}")
            # Ensure checkbox is checked if a cache is selected this way
            if not self.kv_cache_checkbox.isChecked():
                 self.kv_cache_checkbox.setChecked(True) # This will trigger toggle_kv_cache again, but it's idempotent
        else:
            # set_kv_cache emits error signal if path is invalid
            self.kv_cache_label.setText("Current KV Cache: Invalid Selection")
    ...
    def on_response_started(self):
        ...
        cursor.insertHtml(
            f'<div style="margin-top: 10px;">'
            f'<div style="font-weight: bold; color: #388E3C;">Assistant:</div>'
            f'<div id="current-response" style="background-color: #F1F8E9; border-radius: 5px; padding: 8px; margin: 5px 0px; color: #333333;">'
            f'</div></div>'
        )
